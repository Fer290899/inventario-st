    ...
    <Transition name="fade">
      ...
      >
        <Transition name="scale" appear>
          <div v-if="open" class="flex max-h-[90vh] w-full max-w-4xl flex-col rounded-2xl bg-white shadow-2xl">
            <!-- Encabezado -->
            <div class="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 px-6 py-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs text-slate-500">Ajusta los criterios y aplica para filtrar el listado</p>
                </div>
              </div>
              <button
                type="button"
              ...
              <div class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="space-y-4">
                  <div>
                    ...
                    <div class="relative">
                      <select v-model="draft.tipo" class="w-full appearance-none rounded-lg border border-slate-200 bg-slate-50 py-2 pl-3 pr-7 text-sm text-slate-700 outline-none transition focus:border-blue-400/60 focus:bg-white focus:ring-2 focus:ring-blue-500/30">
                  ...
                  <div>
                    ...
                    <div class="relative">
                      <select v-model="draft.estatus" class="w-full appearance-none rounded-lg border border-slate-200 bg-slate-50 py-2 pl-3 pr-7 text-sm text-slate-700 outline-none transition focus:border-blue-400/60 focus:bg-white focus:ring-2 focus:ring-blue-500/30">
                  ...
                  <div>
                    ...
                    <div class="relative">
                      <select v-model="draft.inventariable" class="w-full appearance-none rounded-lg border border-slate-200 bg-slate-50 py-2 pl-3 pr-7 text-sm text-slate-700 outline-none transition focus:border-blue-400/60 focus:bg-white focus:ring-2 focus:ring-blue-500/30">
                ...
                <div class="space-y-4">
                  <div>
                    ...
                    <div class="relative">
                      <select v-model="draft.ubicacion" class="w-full appearance-none rounded-lg border border-slate-200 bg-slate-50 py-2 pl-3 pr-7 text-sm text-slate-700 outline-none transition focus:border-blue-400/60 focus:bg-white focus:ring-2 focus:ring-blue-500/30">
                ...
                <div>
                  ...
                  <div class="space-y-3">
                    <div>
                      ...
                      <input v-model="draft.fechaAltaDesde" type="date" class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-blue-400/60 focus:bg-white focus:ring-2 focus:ring-blue-500/30" />
                    </div>
                    <div>
                      <label class="mb-1 block text-xs font-medium text-slate-500">A</label>
                      <input v-model="draft.fechaAltaHasta" type="date" class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-blue-400/60 focus:bg-white focus:ring-2 focus:ring-blue-500/30" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="flex shrink-0 items-center justify-between gap-2 border-t border-slate-200 px-6 py-4">
              <button type="button" class="text-sm font-medium text-blue-600 transition hover:text-blue-700" @click="draft = filtrosVacios()">
                Limpiar filtros
              </button>
...
// Cada vez que se abre, arranca desde los filtros ya aplicados (no desde lo
// último que se haya escrito en un intento anterior sin aplicar).
watch(open, (isOpen) => {
